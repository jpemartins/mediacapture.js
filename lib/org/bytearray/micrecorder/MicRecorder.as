package org.bytearray.micrecorder
{
	...
	public final class MicRecorder extends EventDispatcher
	{
		...
		private var NSAMPLES:uint = 1;		
		private var _samplesProcessed:uint = 0;
		...
		{	
			...
			var recordingEvent:RecordingEvent = new RecordingEvent( RecordingEvent.RECORDING, 0 );
						
			recordingEvent.time = getTimer() - _difference;
			recordingEvent.diff = recordingEvent.time - _lastTime;
			
			_lastTime = recordingEvent.time;
			...
			if (_samplesProcessed == NSAMPLES){				
				_stream.position = 0;
				recordingEvent.samples = _encoder.encode(_stream, 1);

				_stream = new ByteArray();
				_samplesProcessed = 0;
			}

			dispatchEvent( recordingEvent );
